...
@Injectable()
export class AiServicePrismaService
  extends PrismaClient
  implements OnModuleInit, OnModuleDestroy
{
  private readonly logger = new Logger(AiServicePrismaService.name);

  // Connect to the database when the NestJS module initializes
  async onModuleInit() {
    await this.$connect();
    this.logger.log('AI Service DB connected successfully.');
  }

  // Disconnect from the database when the NestJS application shuts down
  async onModuleDestroy() {
    await this.$disconnect();
    this.logger.log('AI Service DB disconnected.');
  }
}
